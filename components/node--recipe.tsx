...
export function NodeRecipe({ node }: NodeRecipeProps) {
  return (
    <article className="py-10">
      <div className="container flex flex-col max-w-4xl px-4 mx-auto space-y-4">
        <div className="flex flex-col space-y-4">
          <h1 className="text-5xl font-semibold tracking-wide">{node.title}</h1>
          {node.field_summary && (
            <div
              dangerouslySetInnerHTML={{
                __html: node.field_summary.processed,
              }}
              className="text-xl font-light leading-relaxed prose text-black"
            />
          )}
        </div>
        ...
        <div className="grid gap-4 pb-10 lg:grid-cols-4">
          {node.field_preparation_time && (
            <div className="flex items-center space-x-2 lg:flex-col">
              ...
              <p className="lowercase">{node.field_preparation_time} minutes</p>
            </div>
          )}
          {node.field_cooking_time && (
            <div className="flex items-center space-x-2 lg:flex-col">
              ...
              <p className="lowercase">{node.field_cooking_time} minutes</p>
            </div>
          )}
          {node.field_number_of_servings && (
            <div className="flex items-center space-x-2 lg:flex-col">
              ...
              <p className="font-semibold">Number of Servings</p>
        ...
        <div className="grid gap-16 md:grid-cols-3">
          <div className="flex flex-col space-y-6 bg-body">
            <h2 className="pb-3 font-serif text-2xl border-b lg:text-3xl">
              Ingredients
            </h2>
            <ul className="divide-y">
              {node.field_ingredients?.map((ingredients, index) => (
                <li
                  key={index}
                  className="flex items-start justify-start py-3 space-x-2"
                >
                  <svg
                    className="relative w-4 h-4 top-1"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                  ...
                  <p className="flex-1 text-gray-600">{ingredients}</p>
                </li>
              ))}
            </ul>
          </div>
          <div className="flex flex-col space-y-6 md:col-span-2">
            <h3 className="pb-3 font-serif text-2xl border-b lg:text-3xl">
              Recipe Instructions
            </h3>
            {node.field_recipe_instruction && (
              <div
                ...
                className="prose text-black prose-li:marker:text-primary prose-li:marker:text-xl max-w-none text-text"
              />
            )}
          </div>
        </div>
